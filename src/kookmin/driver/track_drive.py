...
def start():
    ...
    #=========================================
    started = False       # 출발 여부
    start_boost = True    # 출발 직후 가속 플래그
    boost_duration = 1.5  # 출발 가속 지속 시간 (초)
    boost_start_time = 0  # 가속 시작 시간 기록용
    ...
    while not rospy.is_shutdown():
        ...
        if not started:
            if detect_green_light(image):
                print("✅ Green light detected! Starting...")
                started = True
                start_boost = True
                boost_start_time = time.time()
                drive(angle=0.0, speed=50.0)  # 출발 가속
            else:
        ...
        if ranges is not None:
            ...
            # 주행 명령
            print(f"▶ steering_angle: {steering_angle:.2f}, speed: {speed:.2f}")
